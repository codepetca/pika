...
test('teacher can evaluate calendar chip movement without live writes', async ({ page }, testInfo) => {
  ...
  await expect(prototype).toHaveAttribute('data-drag-active', 'true')
  await page.keyboard.press('ArrowRight')
  await expect(prototype.getByRole('group', { name: 'Wednesday, September 16, 2026' })).toHaveAttribute('data-drop-target', 'true')
  await page.keyboard.press('ArrowRight')
  await expect(prototype.getByRole('group', { name: 'Thursday, September 17, 2026' })).toHaveAttribute('data-drop-target', 'true')
  await page.keyboard.press('Space')
  await expect(prototype).toHaveAttribute('data-drag-active', 'false')
  ...
  await expect(prototype.getByRole('group', { name: 'Monday, September 14, 2026' })).toHaveAttribute('data-drop-target', 'true')
