...
test('combines Daily logs and entitled Attendance in one teacher work surface', async ({ page }, testInfo) => {
  ...
  await expect(posterDialog.getByText('Check in for attendance')).toHaveClass(/text-3xl/)
  ...
  if (viewport === 'mobile') {
    ...
    await expect(posterDialog.getByText('Check in for attendance')).toBeHidden()
