...
describe('StudentTestsTab exam mode', () => {
  ...
  it('uses centered detail before start and 30/70 exam mode split after start', async () => {
    ...
    expect(splitContainerAfterStart.className).toContain('lg:grid-cols-[minmax(0,3fr)_minmax(0,7fr)]')
    expect(splitContainerAfterStart.className).not.toContain('lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]')
    ...
    const documentButton = within(leftPane).getByRole('button', { name: 'Node.js API' })
    expect(documentButton).toBeInTheDocument()
    expect(within(documentButton).getByTestId('student-test-document-open-icon')).toBeInTheDocument()
  ...
  it('switches to 50/50 split when opening a doc and restores 30/70 on back', async () => {
    ...
    expect(splitContainerDocOpen.className).toContain('lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]')
    expect(splitContainerDocOpen.className).not.toContain('lg:grid-cols-[minmax(0,3fr)_minmax(0,7fr)]')
    ...
    expect(splitContainerBack.className).toContain('lg:grid-cols-[minmax(0,3fr)_minmax(0,7fr)]')
  })
  ...
  it('shows Closed, Submitted, and Returned status pills for tests', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Individually Closed Test')).toBeInTheDocument()
      expect(screen.getByText('Opened Published Test')).toBeInTheDocument()
    })

    const closedCard = screen.getByRole('button', { name: /^Closed Test\b/i })
