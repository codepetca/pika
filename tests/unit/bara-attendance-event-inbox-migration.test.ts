...
describe('Bara attendance event inbox migration', () => {
  ...
  it('creates a private leased Pika-to-Bara delivery outbox', () => {
    ...
    expect(migration).toContain('perform pg_advisory_xact_lock(hashtextextended(p_classroom_id::text, 918273645))')
    expect(migration).toContain("sibling.status in ('pending', 'processing')")
    expect(migration).toContain("sibling.payload->>'occurrence_ref' = p_row.payload->>'occurrence_ref'")
