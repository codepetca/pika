...
describe('course blueprint package docs', () => {
  it('documents the official portable package contract and round trip', () => {
    ...
    expect(docs).toContain('classwork-materials.md')
    expect(docs).toContain('surveys.md')
    ...
    expect(docs).toContain('Raw schemas never create missing files or supply defaults')
    expect(docs).toContain('Immutable JSON and binary TAR fixtures')
    expect(docs).toContain('Optional `quizzes.md`, which is discarded')
    ...
    expect(lifecycleDocs).toContain('Import accepts versions 2, 3, 4, and')
  })
})
