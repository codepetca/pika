...
describe('course blueprint package docs', () => {
  it('documents the official portable package contract and round trip', () => {
    ...
    expect(docs).toContain('canonical export manifest version is `4`')
    expect(docs).toContain('imports versions `3` and')
    expect(lifecycleDocs).toContain('export manifest is version 4')
    expect(lifecycleDocs).toContain('Import accepts versions 3 and 4')
  })
})
