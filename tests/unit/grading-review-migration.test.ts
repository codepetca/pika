...
describe('teacher grading review migration', () => {
  ...
  it('captures assignment suggestions and finalizes them when teachers return work', () => {
    ...
    expect(migration).toContain('if new.score_completion is null')
    expect(migration).toContain('or new.score_thinking is null')
    expect(migration).toContain('or new.score_workflow is null')
  ...
  it('keeps database-backed acceptance, correction, dismissal, and privacy checks in CI', () => {
    ...
    expect(assignmentHarness).toContain('provenance without completed scores to omit the grading review snapshot')
