...
describe('attendance entitlement operator command', () => {
  ...
  it('launches the outbox recovery command before validating input', () => {
    const command = process.platform === 'win32' ? 'cmd.exe' : 'pnpm'
    const commandArguments = process.platform === 'win32'
      ? ['/d', '/s', '/c', 'pnpm attendance:outbox:recover-epoch -- --operation-id invalid']
      : ['attendance:outbox:recover-epoch', '--', '--operation-id', 'invalid']
    const result = spawnSync(
      command,
      commandArguments,
      {
