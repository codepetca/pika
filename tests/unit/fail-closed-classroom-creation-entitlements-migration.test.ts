...
describe('fail-closed classroom creation entitlement migration', () => {
  it('keeps existing accounts compatible while provisioning future accounts as Free', () => {
  ...
  it('serializes signup and creation with the one-way cutover boundary', () => {
    ...
    expect(migration).toMatch(
      /assert_classroom_creation_allowed_v1[\s\S]*classroom_creation_entitlement_settings[\s\S]*for share[\s\S]*lock_effective_feature_entitlement_v1/,
    )
    expect(migration).toMatch(
