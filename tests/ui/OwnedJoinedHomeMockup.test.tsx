...
describe('OwnedJoinedHomeMockup', () => {
  ...
  it('keeps a keyboard anchor when an archived row or the back action disappears', async () => {
    ...
    expect(home.getByRole('heading', { name: 'Active classrooms' })).toHaveFocus()
  })

  it('matches list Escape and checked menu state without stealing nested Escape', async () => {
    const { user, home } = setup()
    ...
    expect(home.getByRole('button', { name: 'Back to classrooms' })).toBeVisible()
    await user.click(home.getByRole('button', { name: 'Archive Grade 10 Science' }))
    await user.keyboard('{Escape}')
    expect(home.getByRole('button', { name: 'Archive Grade 10 Science' })).toHaveFocus()
    expect(home.getByRole('button', { name: 'Back to classrooms' })).toBeVisible()
    // Exercise the list itself, without the restored icon's tooltip owning Escape.
    await user.click(home.getByRole('heading', { name: 'Active classrooms' }))
    await user.keyboard('{Escape}')
    expect(home.queryByRole('button', { name: 'Back to classrooms' })).not.toBeInTheDocument()
    expect(home.getByRole('heading', { name: 'Active classrooms' })).toHaveFocus()
    expect(home.getByRole('button', { name: 'All', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await user.click(actions)
