...
describe('OwnedJoinedHomeMockup', () => {
  ...
  it('shows both relationships without a redundant filter or account-type change', () => {
    const { home } = setup()
    ...
    expect(home.queryByRole('button', { name: 'All', exact: true })).not.toBeInTheDocument()
    expect(home.queryByRole('button', { name: 'Teaching', exact: true })).not.toBeInTheDocument()
    expect(home.queryByRole('button', { name: 'Joined', exact: true })).not.toBeInTheDocument()
  })
  ...
  it('lets a joined-only account hide its last class and find it again', async () => {
    ...
    await user.click(screen.getByRole('menuitemcheckbox', { name: 'Edit classrooms' }))
    await user.click(home.getByRole('button', { name: 'Hide Learning Design' }))
    expect(home.getByText('No active classrooms')).toBeVisible()
    await user.click(actions)
    await user.click(screen.getByRole('menuitem', { name: 'Show Archived' }))
  ...
  it('matches list Escape and checked menu state without stealing nested Escape', async () => {
    ...
    expect(home.getByRole('heading', { name: 'Active classrooms' })).toHaveFocus()
    expect(home.getByRole('region', { name: 'Teaching classrooms' })).toBeVisible()
