...
describe('PageMockups', () => {
  it('opens the standalone item editor with deterministic defaults and restores focus on Escape', async () => {
    ...
    expect(gradebook.queryByRole('button', { name: 'Add other assessment' })).not.toBeInTheDocument()
    const opener = gradebook.getByRole('button', { name: 'More actions' })
    await user.click(opener)
    await user.keyboard('{Home}')
    expect(screen.getByRole('menuitem', { name: 'Edit categories' })).toHaveFocus()
    await user.keyboard('{ArrowDown}{Enter}')
    const dialog = within(screen.getByRole('dialog', { name: 'Add other assessment' }))
    expect(dialog.getByRole('textbox', { name: 'Assessment title' })).toHaveValue('')
    ...
    expect(dialog.getByRole('button', { name: 'Add other assessment' })).toBeDisabled()
    await user.keyboard('{Escape}')
    expect(screen.queryByRole('dialog', { name: 'Add other assessment' })).not.toBeInTheDocument()
  ...
  it('renders the production-shaped Daily controls with deterministic attendance interactions', async () => {
    ...
    const dateControl = within(daily).getByRole('button', { name: 'Return to reference Daily date' })
    expect(dateControl).toHaveAccessibleDescription('Today')
    await user.click(within(daily).getByRole('button', { name: 'More actions' }))
    await user.click(within(daily).getByRole('menuitemcheckbox', { name: 'Hide relative date' }))
    expect(dateControl).not.toHaveAttribute('aria-describedby')
    expect(dateControl.querySelector('[aria-hidden="true"]')).toBeNull()
    expect(dateControl).not.toHaveClass('flex-col')
    await user.click(within(daily).getByRole('button', { name: 'More actions' }))
    await user.click(within(daily).getByRole('menuitemcheckbox', { name: 'Show relative date' }))
    expect(dateControl).toHaveAccessibleDescription('Today')
    expect(dateControl).toHaveClass('flex-col')
    ...
    const logHeader = within(daily).getByRole('columnheader', { name: /^Log/ })
    const presentHeader = within(daily).getByRole('columnheader', { name: /present/i })
    ...
    const openQr = within(daily).getByRole('button', { name: 'Check in for attendance' })
    expect(openQr).toBeEnabled()
    await user.click(openQr)
    const qrDialog = screen.getByRole('dialog', { name: 'Check in for attendance' })
    expect(qrDialog).toHaveClass('aspect-[2/3]', 'sm:aspect-video')
    expect(within(qrDialog).getByLabelText('Environmental Science check in for attendance QR code')).toBeVisible()
    ...
    expect(within(qrDialog).getAllByText('Check in for attendance').find((node) => node.tagName === 'P'))
      .toHaveClass('sm:text-3xl')
    ...
    expect(within(qrDialog).getByRole('button', { name: 'QR options' })).toBeVisible()
    await user.click(within(qrDialog).getByRole('button', { name: 'Poster settings' }))
    const posterSettings = screen.getByRole('menu', { name: 'Poster settings' })
    expect(within(posterSettings).getByRole('menuitem', { name: 'Print poster' })).toBeVisible()
    expect(within(posterSettings).getByRole('menuitem', { name: 'Download SVG' })).toBeVisible()
    await user.click(within(posterSettings).getByRole('menuitem', { name: 'Rotate QR' }))
    ...
    await user.click(within(qrDialog).getByRole('button', { name: 'Close' }))
    ...
    expect(await screen.findByRole('tooltip')).toHaveTextContent('Check in for attendance')
    ...
    expect(within(daily).queryByRole('button', { name: 'Undo override for Maya Chen' })).not.toBeInTheDocument()
    const noahUndo = within(daily).getByRole('button', { name: 'Undo override for Noah Williams' })
    const noahAbsent = within(daily).getByRole('button', { name: 'Mark Noah Williams absent' })
    const noahPresent = within(daily).getByRole('button', { name: 'Mark Noah Williams present' })
    const noahLate = within(daily).getByRole('button', { name: 'Mark Noah Williams late' })
    const presentSort = within(daily).getByRole('button', { name: 'Sort Present first, 2 students' })
    expect(noahUndo).toBeVisible()
    // The mockup must pin the same status columns as the production Daily table.
    expect(noahAbsent.closest('td')).toHaveClass('sticky', 'right-attendance-one')
    expect(noahLate.closest('td')).toHaveClass('sticky', 'right-attendance-two')
    expect(noahPresent.closest('td')).toHaveClass('sticky', 'right-attendance-three')
    expect(noahUndo.closest('td')).toHaveClass('sticky', 'right-0')
    expect(noahUndo.closest('td')?.cellIndex).toBe((noahAbsent.closest('td')?.cellIndex ?? 0) + 1)
    await user.hover(noahUndo)
    expect(await screen.findByRole('tooltip')).toHaveTextContent(/^Undo override$/)
    ...
    expect(within(daily).getByRole('button', { name: 'Check in for attendance' })).toBeEnabled()
    ...
    expect(within(daily).getByRole('button', { name: 'Undo override for Maya Chen' })).toBeVisible()
    ...
    expect(within(daily).queryByRole('button', { name: /Undo override/ })).not.toBeInTheDocument()
  ...
  it('switches Daily to passive manual attendance without QR check-in evidence', async () => {
    ...
    expect(within(daily).queryByRole('button', { name: 'Check in for attendance' })).not.toBeInTheDocument()
    ...
    expect(within(daily).getByRole('button', { name: 'Undo override for Noah Williams' })).toBeVisible()
    expect(within(daily).getByRole('button', { name: 'Undo override for Sana Patel' })).toBeVisible()
  ...
  it('shows class summaries in the action bar and uses a per-student mobile surface', async () => {
    const user = userEvent.setup()
    renderMockups()
    const mockups = screen.getByTestId('page-mockups')
    await user.click(within(mockups).getByRole('tab', { name: 'Gradebook' }))
    ...
    expect(scrollFrame).toHaveClass('hidden', 'lg:block', 'h-80', 'overflow-auto')
    expect(within(gradebook).getByLabelText('Class Average 84.6% · Median 86%')).toHaveTextContent('Class Average 84.6% · Median 86%')
    expect(within(gradebook).queryByRole('group', { name: 'Class summary' })).not.toBeInTheDocument()
    expect(within(gradebook).getByRole('row', { name: 'Class average' })).toHaveTextContent('Avg85%83%')
  ...
  it('shows category, weight, and course percent rows below the headers', async () => {
    const user = userEvent.setup()
    renderMockups()
    const mockups = screen.getByTestId('page-mockups')
    await user.click(within(mockups).getByRole('tab', { name: 'Gradebook' }))
    ...
    const categoryRow = within(gradebook).getByRole('row', { name: 'Category' })
    const weightRow = within(gradebook).getByRole('row', { name: 'Weight' })
    const coursePercentRow = within(gradebook).getByRole('row', { name: 'Course %' })
    expect(categoryRow).toHaveTextContent('CategoryTermTerm')
    const ecosystemWeight = within(weightRow).getByRole('spinbutton', { name: 'Category weight for Ecosystems' })
    expect(ecosystemWeight).toHaveValue(10)
    expect(within(coursePercentRow).getByLabelText('Course weight for Ecosystems')).toHaveTextContent('5.42%')

    await user.click(within(categoryRow).getByRole('button', { name: 'Edit category for A1: Ecosystems' }))
    expect(screen.getByRole('heading', { name: 'Edit assessment' })).toBeInTheDocument()
    ...
    expect(within(coursePercentRow).getByLabelText('Course weight for Ecosystems')).toHaveTextContent('10%')

    await user.click(showWeights)
    expect(showWeights).toHaveAttribute('aria-pressed', 'false')
    expect(within(gradebook).queryByRole('row', { name: 'Category' })).not.toBeInTheDocument()
    expect(within(gradebook).queryByRole('row', { name: 'Weight' })).not.toBeInTheDocument()
    expect(within(gradebook).queryByRole('row', { name: 'Course %' })).not.toBeInTheDocument()
  })

  it('edits an individual mark and supports per-cell and undo-all actions', async () => {
    const user = userEvent.setup()
    renderMockups()
    const mockups = screen.getByTestId('page-mockups')
    await user.click(within(mockups).getByRole('tab', { name: 'Gradebook' }))
    ...
    expect(within(gradebook).queryByRole('button', { name: 'Undo all overrides' })).not.toBeInTheDocument()
    expect(within(gradebook).getByRole('button', { name: 'Edit Noah Williams-Montgomery mark for Genetics: 68%' }).querySelector('span')).toHaveClass('text-warning')
    await user.click(within(gradebook).getByRole('button', { name: 'Edit Maya Chen mark for Ecosystems: 90%' }))
    expect(screen.getByRole('heading', { name: 'Edit mark' })).toBeInTheDocument()
    expect(screen.getByText('Maya Chen · Ecosystems')).toBeInTheDocument()
    const mark = screen.getByRole('spinbutton', { name: 'Mark earned' })
    expect(mark).toHaveAttribute('step', '0.1')
    expect(mark).toHaveClass('w-24', '[appearance:textfield]')
    await user.clear(mark)
    await user.type(mark, '21')
    expect(screen.getByRole('status')).toHaveTextContent('This mark is 1 point over the total of 20.')
    expect(screen.getByRole('button', { name: 'Save mark' })).toBeEnabled()
    await user.clear(mark)
    await user.type(mark, '19.05')
    expect(screen.getByRole('button', { name: 'Save mark' })).toBeDisabled()
    await user.clear(mark)
    await user.type(mark, '19')
    await user.click(screen.getByRole('button', { name: 'Save mark' }))

    const modifiedMark = within(gradebook).getByRole('button', { name: 'Edit Maya Chen mark for Ecosystems: 95%, overridden' })
    expect(modifiedMark).not.toHaveClass('text-primary')
    expect(modifiedMark.querySelector('svg')).toHaveClass('text-primary', 'h-3', 'w-3')
    expect(modifiedMark.querySelector('span:last-child')).toHaveClass('text-text-default')
    expect(within(gradebook).queryByRole('button', { name: /Undo override for Maya Chen/ })).not.toBeInTheDocument()
    await user.click(modifiedMark)
    const overrideDialog = screen.getByRole('dialog')
    expect(within(overrideDialog).getByRole('button', { name: 'Undo override' })).toBeInTheDocument()
    expect(within(overrideDialog).getByLabelText('Mark after undo: 18')).toHaveTextContent('18')
    await user.click(within(overrideDialog).getByRole('button', { name: 'Undo override' }))
    expect(overrideDialog).toBeInTheDocument()
    expect(within(overrideDialog).getByRole('spinbutton', { name: 'Mark earned' })).toHaveValue(18)
    expect(within(overrideDialog).queryByRole('button', { name: 'Undo override' })).not.toBeInTheDocument()
    expect(within(overrideDialog).getByRole('button', { name: 'Save mark' })).toBeDisabled()
    await user.click(within(overrideDialog).getByRole('button', { name: 'Cancel' }))
    await user.click(within(gradebook).getByRole('button', { name: 'Edit Maya Chen mark for Ecosystems: 90%' }))
    const replacementMark = screen.getByRole('spinbutton', { name: 'Mark earned' })
    await user.clear(replacementMark)
    await user.type(replacementMark, '19')
    await user.click(screen.getByRole('button', { name: 'Save mark' }))
    await user.click(within(gradebook).getByRole('button', { name: 'More actions' }))
    await user.click(screen.getByRole('menuitem', { name: 'Undo all overrides' }))
    expect(screen.getByRole('dialog', { name: 'Undo all overrides?' })).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Undo all' }))
    await user.click(within(gradebook).getByRole('button', { name: 'More actions' }))
    expect(screen.queryByRole('menuitem', { name: 'Undo all overrides' })).not.toBeInTheDocument()
    await user.keyboard('{Escape}')
    expect(within(gradebook).getByRole('button', { name: 'Edit Maya Chen mark for Ecosystems: 90%' })).toBeInTheDocument()

    await user.click(within(gradebook).getByRole('button', { name: 'Edit Maya Chen final mark: 86%' }))
    const finalMarkDialog = screen.getByRole('dialog')
    const finalMarkInput = within(finalMarkDialog).getByRole('spinbutton', { name: 'Final mark' })
    expect(finalMarkInput).toHaveAttribute('step', '0.1')
    expect(within(finalMarkDialog).getByText('%')).toBeInTheDocument()
    await user.clear(finalMarkInput)
    await user.type(finalMarkInput, '49')
    await user.click(screen.getByRole('button', { name: 'Save mark' }))
    const overriddenFinal = within(gradebook).getByRole('button', { name: 'Edit Maya Chen final mark: 49%, overridden' })
    expect(overriddenFinal.querySelector('svg')).toHaveClass('text-primary', 'h-3', 'w-3')
    expect(overriddenFinal.querySelector('span:last-child')).toHaveClass('text-danger')
    await user.click(overriddenFinal)
    const finalOverrideDialog = screen.getByRole('dialog')
    expect(within(finalOverrideDialog).getByLabelText('Mark after undo: 86%')).toHaveTextContent('86%')
    await user.click(within(finalOverrideDialog).getByRole('button', { name: 'Undo override' }))
    expect(finalOverrideDialog).toBeInTheDocument()
    expect(within(finalOverrideDialog).getByRole('spinbutton', { name: 'Final mark' })).toHaveValue(86)
    expect(within(finalOverrideDialog).queryByRole('button', { name: 'Undo override' })).not.toBeInTheDocument()
    await user.click(within(finalOverrideDialog).getByRole('button', { name: 'Cancel' }))
  })
  ...
  it('uses a top-right classroom menu and Escape returns to the active non-editing list', async () => {
    ...
    const settings = within(classrooms).getByRole('button', { name: 'Settings for Earth and Space Science' })
    await user.click(settings)
    expect(within(classrooms).getByRole('menuitem', { name: 'Reuse' })).toHaveFocus()
    expect(within(classrooms).getByRole('menuitem', { name: 'Unarchive' })).toBeVisible()
    expect(within(classrooms).getByRole('menuitem', { name: 'Delete', exact: true })).toBeVisible()
    await user.keyboard('{Escape}')
    expect(settings).toHaveFocus()
    expect(within(classrooms).getByRole('heading', { name: 'Archived classrooms' })).toBeVisible()
  ...
  it('exercises WorkSurfaceMockup from a full-width list through its active inspector', async () => {
    ...
    await user.click(within(workspace).getByRole('button', { name: 'More actions' }))
    const editTests = within(workspace).getByRole('menuitemcheckbox', { name: 'Edit Tests' })
    expect(editTests).toHaveAttribute('aria-checked', 'false')
