...
describe('PageMockups', () => {
  ...
  it('pins one class summary row and swaps average with median from the center controls', async () => {
    ...
    expect(within(gradebook).getByText(/Center buttons toggle score display/)).toBeInTheDocument()

    const summaryControl = within(gradebook).getByRole('group', { name: 'Class summary' })
    expect(within(summaryControl).getAllByRole('button')).toHaveLength(1)
    await user.click(within(summaryControl).getByRole('button', { name: 'Class summary: AVG. Switch to MED' }))
    ...
    await user.click(within(summaryControl).getByRole('button', { name: 'Class summary: MED. Switch to AVG' }))
    const scoreControl = within(gradebook).getByRole('group', { name: 'Score display' })
    expect(within(scoreControl).getAllByRole('button')).toHaveLength(1)
    await user.click(within(scoreControl).getByRole('button', { name: 'Score display: %. Switch to x/y' }))
    expect(within(scoreControl).getByRole('button', { name: 'Score display: x/y. Switch to %' })).toHaveTextContent('x/y')
