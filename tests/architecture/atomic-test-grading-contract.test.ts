...
describe('atomic test grading contract', () => {
  ...
  it('locks a deterministic response set and rejects stale batch members transactionally', () => {
    ...
    expect(databaseHarness).toContain('Question wording/grading ordering was not preserved')
    expect(databaseHarness).toContain('2:5.00:Question edited without a lock cycle')
