...
import type { PalErasureReceipt } from '@/lib/server/pal-profile-erasure'
...
describe('durable provider prerequisite coordination', () => {
  it('is disabled before any persistence or transport', async () => {
    ...
    await expect(f.coordinator.advance(scope)).rejects.toMatchObject({ code: 'disabled' })
  ...
  it('cannot send without an authorized durable binding', async () => {
    const f = fixture()
    await expect(f.coordinator.advance(scope)).rejects.toMatchObject({ code: 'persistence_unavailable' })
  ...
  it('reserves without HTTP and makes only one provider attempt per advance', async () => {
    ...
    expect(f.pal).not.toHaveBeenCalled(); expect(f.bara).not.toHaveBeenCalled()
    await f.coordinator.advance(scope)
    ...
    expect(await f.coordinator.advance(scope)).toMatchObject({ status: 'provider_pending', pal: 'completed', bara: 'deleted', cleanup_completed: false })
  ...
  it.each(['operation_id','generation_id','pal_reference'] as const)('rejects wrong saved %s before networking', async field => {
    ...
    await expect(f.coordinator.advance(scope)).rejects.toMatchObject({ code: 'binding_invalid' })
  ...
  it('keeps managed-copy pending Pal state pending and retains its fence identity', async () => {
    ...
    expect(await f.coordinator.advance(scope)).toMatchObject({ pal: 'pending', cleanup_completed: false })
    expect(f.bara).not.toHaveBeenCalled()
