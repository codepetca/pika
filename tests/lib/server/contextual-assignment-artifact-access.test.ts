...
import { authorizeContextualAssignmentArtifactRequest } from '@/lib/server/contextual-assignment-doc-access'
...
describe('contextual assignment artifact access', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_ARTIFACT_ACCESS_ENABLED', 'true')
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_ARTIFACT_ACCESS_PAIRS', JSON.stringify([{
      userId: actorId,
      assignmentId,
    }]))
  ...
  it('preserves legacy auth-before-params behavior when disabled', async () => {
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_ARTIFACT_ACCESS_ENABLED', 'false')
    const resolveId = vi.fn(async () => assignmentId)

    await expect(authorizeContextualAssignmentArtifactRequest(resolveId)).resolves.toEqual({
      mode: 'legacy',
      user: student,
      assignmentId,
    })
  ...
  it('admits an exact teacher-valued pair without cross-producting the cohort', async () => {
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_ARTIFACT_ACCESS_PAIRS', JSON.stringify([
      { userId: actorId, assignmentId },
      { userId: otherActorId, assignmentId: otherAssignmentId },
    ]))

    await expect(authorizeContextualAssignmentArtifactRequest(assignmentId.toUpperCase()))
      .resolves.toEqual({ mode: 'contextual', user: teacher, assignmentId })
    await expect(authorizeContextualAssignmentArtifactRequest(otherAssignmentId))
  ...
  it.each([
    ...
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_ARTIFACT_ACCESS_PAIRS', pairs)
    await expect(authorizeContextualAssignmentArtifactRequest(assignmentId))
      .rejects.toMatchObject({ statusCode: 503 })
  })

  it('keeps an unmatched student on the legacy path', async () => {
    vi.mocked(requireAuth).mockResolvedValue(student as any)
    await expect(authorizeContextualAssignmentArtifactRequest(otherAssignmentId)).resolves.toEqual({
      mode: 'legacy',
      user: student,
