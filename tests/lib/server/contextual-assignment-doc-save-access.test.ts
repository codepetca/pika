...
import { authorizeContextualAssignmentDocSaveRequest } from '@/lib/server/contextual-assignment-doc-access'
...
describe('contextual assignment document save access', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('preserves legacy auth-before-params behavior when disabled', async () => {
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_DOC_SAVE_ACCESS_ENABLED', 'false')
    const resolveId = vi.fn(async () => assignmentId)

    await expect(authorizeContextualAssignmentDocSaveRequest(resolveId)).resolves.toEqual({
      mode: 'legacy',
      user: student,
      assignmentId,
    })
  ...
  it('admits an exact teacher-valued pair without cross-producting the cohort', async () => {
    ...
    await expect(authorizeContextualAssignmentDocSaveRequest(assignmentId.toUpperCase()))
      .resolves.toEqual({ mode: 'contextual', user: teacher, assignmentId })
    await expect(authorizeContextualAssignmentDocSaveRequest(otherAssignmentId))
  ...
  it.each([
    ...
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_DOC_SAVE_ACCESS_PAIRS', pairs)
    await expect(authorizeContextualAssignmentDocSaveRequest(assignmentId))
      .rejects.toMatchObject({ statusCode: 503 })
  })

  it('keeps an unmatched student on the legacy path', async () => {
    vi.mocked(requireAuth).mockResolvedValue(student as any)
    await expect(authorizeContextualAssignmentDocSaveRequest(otherAssignmentId)).resolves.toEqual({
      mode: 'legacy',
      user: student,
