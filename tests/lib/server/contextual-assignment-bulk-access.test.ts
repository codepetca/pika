...
describe('contextual Assignment bulk access', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_ENABLED', 'true')
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_PAIRS', JSON.stringify([{ userId: actorId, classroomId }]))
  ...
  it('preserves teacher auth before resolving the Classroom when disabled', async () => {
    vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_ENABLED', 'false')
    const resolveId = vi.fn(async () => classroomId)
    await expect(authorizeContextualAssignmentBulkRequest(resolveId))
  ...
  it.each([teacher, student])('admits an exact pair for a $role-valued owner', async (user) => {
    ...
    await expect(authorizeContextualAssignmentBulkRequest(classroomId.toUpperCase()))
  ...
  it('keeps an unmatched teacher legacy and denies an unmatched student', async () => {
    await expect(authorizeContextualAssignmentBulkRequest(otherClassroomId))
  ...
  it.each(['', 'not-json', '{}', '[{"userId":"*","classroomId":"*"}]', ' '.repeat(20_001)])(
    ...
    async (pairs) => {
      vi.stubEnv('PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_PAIRS', pairs)
      await expect(authorizeContextualAssignmentBulkRequest(classroomId))
