...
describe('POST /api/teacher/tests/[id]/questions/reorder', () => {
  ...
  it('retires direct row reordering in favor of a versioned draft edit', async () => {
    const response = await POST(
      new NextRequest('http://localhost:3000/api/teacher/tests/test-1/questions/reorder', {
        method: 'POST',
        body: JSON.stringify({ question_ids: ['portable-b', 'portable-a'] }),
      }),
