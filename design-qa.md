...
- Responsiveness and accessibility: desktop/mobile light/dark captures have no document overflow. Row status controls use the explicitly requested compact 32 x 32 target treatment while retaining named pressed states, visible focus rings, roving focus, and Arrow/Home/End behavior. Mobile keeps Attendance hours in the condensed action menu. The internal roster scroller retains sticky sortable/resizable headers.
...
- Reduced row status targets from 36 px to 32 px and visible discs/count pills from 32 px to 28 px; reduced the QR-correction Undo target to the same compact size.
