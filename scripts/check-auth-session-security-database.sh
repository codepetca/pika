...
docker exec "$DB_CONTAINER" psql -U postgres -d postgres -X -q -v ON_ERROR_STOP=1 \
  -c "delete from public.users where id = '$USER_ID';
      ...
      begin
        ...
        if not has_table_privilege('service_role', 'public.auth_sessions', 'select,insert,delete') then
...
docker exec "$DB_CONTAINER" psql -U postgres -d postgres -X -q -v ON_ERROR_STOP=1 \
  -c "insert into public.verification_codes (
      ...
      insert into public.auth_sessions (
        ...
        ('$USER_ID', repeat('1', 64), 'password', clock_timestamp() + interval '1 day'),
        ('$USER_ID', repeat('2', 64), 'password', clock_timestamp() + interval '1 day');" >/dev/null

reset_result="$(service_rpc "select public.consume_password_reset_and_revoke_sessions(
  '$USER_ID', '$HANDOFF_HASH', 'new-password-hash'
);")"
if [[ "$reset_result" != "t" ]]; then
...
retry_result="$(service_rpc "select public.consume_password_reset_and_revoke_sessions(
  '$USER_ID', '$HANDOFF_HASH', 'unexpected-password-hash'
);")"
if [[ "$retry_result" != "f" ]]; then
  echo "Consumed password reset handoff was replayable." >&2
  exit 1
fi

echo "Authentication session revocation, rate limiting, privileges, and reset atomicity verified."
