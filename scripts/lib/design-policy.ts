...
const rawColorClassPattern =
  /(?<![\w-])(?:bg|text|border|ring|outline|fill|stroke)-(?:black|white|slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)(?:-[0-9]{2,3})?(?:\/[0-9]+)?(?![\w-])|(?<![\w-])(?:bg|text|border|ring|outline|fill|stroke)-\[(?!(?:var|theme)\()[^\]\n]+\]|(?<![\w-])\[(?:color|background(?:-color)?|border(?:-(?:top|right|bottom|left))?-color|outline-color|fill|stroke):(?!(?:var|theme)\()[^\]\n]+\]/g

const rawCssColorPattern =
  /#[0-9a-fA-F]{3,8}\b|(?:rgb|hsl)a?\(|\b(?:color|background|backgroundColor|borderColor|outlineColor|fill|stroke)\s*:\s*['"`](?!(?:var\(|currentColor|inherit|initial|unset)[^'"`]*['"`])[^'"`\n]+['"`]|\b(?:color|background|background-color|border(?:-(?:top|right|bottom|left))?-color|outline-color|fill|stroke)\s*:\s*(?!(?:var\(|currentColor|inherit|initial|unset)\b)[a-zA-Z][\w-]*(?=\s*;)/g
