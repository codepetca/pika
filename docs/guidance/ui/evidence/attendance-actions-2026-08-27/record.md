...
|---|---|---:|---|---|---|
| `approved-design-reference.png` | teacher | 1440 x 1064 | light | closed/open comparison | final Product Design exploration with a leading, content-sized session time and compact row controls |
| `teacher-desktop-light-default.png` | teacher | 1440 x 900 | light | 45-student default roster | teacher Attendance |
| `teacher-desktop-light-closed.png` | teacher | 1440 x 900 | light | closed session; neutral time control | teacher Attendance |
...
| `teacher-desktop-light-hours-dialog.png` | teacher | 1440 x 900 | light | Attendance timing dialog | teacher Attendance |
| `teacher-desktop-light-selected-check-in.png` | teacher | 1440 x 900 | light | checked-in student selected; automatic/remove actions visible | teacher Attendance |
| `teacher-desktop-dark-default.png` | teacher | 1440 x 900 | dark | 45-student default roster | teacher Attendance |
| `teacher-desktop-dark-closed.png` | teacher | 1440 x 900 | dark | closed session; neutral time control | teacher Attendance |
...
| `teacher-desktop-dark-hours-dialog.png` | teacher | 1440 x 900 | dark | Attendance timing dialog | teacher Attendance |
| `teacher-desktop-dark-selected-check-in.png` | teacher | 1440 x 900 | dark | checked-in student selected; automatic/remove actions visible | teacher Attendance |
...
| `teacher-mobile-light-hours-dialog.png` | teacher | 390 x 844 | light | Attendance timing dialog | teacher Attendance |
| `teacher-mobile-light-hours-dialog-actions.png` | teacher | 390 x full page | light | complete Attendance timing actions | teacher Attendance |
| `teacher-mobile-light-selected-check-in.png` | teacher | 390 x 844 | light | checked-in student selected; automatic/remove actions visible | teacher Attendance |
...
| `teacher-mobile-dark-hours-dialog.png` | teacher | 390 x 844 | dark | Attendance timing dialog | teacher Attendance |
| `teacher-mobile-dark-hours-dialog-actions.png` | teacher | 390 x full page | dark | complete Attendance timing actions | teacher Attendance |
| `teacher-mobile-dark-selected-check-in.png` | teacher | 390 x 844 | dark | checked-in student selected; automatic/remove actions visible | teacher Attendance |
...
- Design claim being checked: the joined date navigator, immediate session actions, and persistent selected-student menu form the dominant centered cluster, while the content-sized session-time control and quiet refresh utility stay at opposite edges above an internally scrolling roster with checkbox selection and inline row corrections.
- Confirmed invariants: previous/date/next controls are contiguous; the selectable date has no dropdown chevron; each student has a checkbox and the header has a select-all checkbox; the selected-student menu stays visible but disabled with no selection and exposes the selected count when enabled; whole-roster status buttons and a manual-editing mode toggle are absent; per-student controls always expose three named pressed states in fixed Present/Late/Absent order with Pika's 44 px direct-action targets around 28 px icon-free visible discs; the selected status has a semantic blue ring while inactive states remain visible at 12% opacity; 28 px count pills align with the status discs; the trailing header shows sortable Present/Late/Absent counts without a visible `Status` label; QR-origin corrections expose a 44 px Undo action that restores the timing-derived automatic status; selected students can also use automatic status or remove QR check-ins from the same persistent menu; Check-in shows the QR time or remains visually empty; the clickable session range shrinks to its content in the leading context slot while accommodating `12:45 AM - 10:34 PM`, uses uppercase AM/PM, and keeps spaces around the dash; only a confirmed open session receives the subtle success background, while closed, stale, and pending states remain neutral and the accessible name retains the state; the old trailing hours icon is absent and a clock control appears when no range exists; mobile session actions collapse without losing QR/open/stop/timing/refresh access; utilities remain reachable; the roster scrolls internally with sticky sortable/resizable headers; there is no page overflow or action overlap.
- Inconsistencies or migration debt: none identified for this scope.
- Intentional differences: the implementation retains Attendance terminology, existing QR/session command rules, Pika typography/tokens, real command-confirmation polling, and existing permission gates. It projects the original QR check-in time plus provider-neutral `hasQrCheckIn`/`hasManualOverride` state from Pika's durable check-in facts so a later staff correction does not erase the provenance needed by Undo; no opaque check-in reference crosses the browser boundary. Mobile moves Attendance timing into its condensed session-actions menu while preserving the centered date/selection hierarchy at 390 px.
...
- Semantic state covered by tests: yes; checkbox state, selected-row state, menu disabled/enabled state, selected count, pressed states, Undo labels, tooltips, accessible group names, absence of row-button icons, 44 px direct-action geometry, 28 px disc/count geometry, selected ring/shadow, inactive opacity, leading longest-range placement, open-only semantic background, neutral closed/stale/pending treatment, accessible session-state naming, and uppercase AM/PM are asserted
