...
- **Primary signal:** the teacher's settings-style visibility switch with a semantic
  DoorOpen/DoorClosed state icon and the
...
|---|---|---|---|
| Teacher visibility control | Gradebook action bar and `SettingsSwitch` | extend | The feature-owned control uses the shared settings switch beside More actions, reinforced by a red DoorClosed or green DoorOpen state icon, a stable accessible name, and a concise show/hide tooltip. |
