...
| --- | --- | --- | --- |
| Daily with QR Attendance | Date plus an equal-height joined QR/time control; session and class-wide commands live in More actions | Daily Log plus First, Last, ID, a tooltip-backed clock header for Time of scan, and tight far-right Present/Late/Absent columns; no row-selection column; an untitled trailing Undo action appears only for manual overrides | Present/Late/Absent counts remain inside their fixed trailing headers; Unmarked has no control; the stable classroom QR remains available while attendance is scheduled or closed because scan-time eligibility is enforced server-side |
| Daily with Pika Manual Attendance | Date plus an optional passive time control; the off-by-default Attendance from log checkbox, time, and class-wide commands live in More actions | Daily Log plus First, Last, ID, and the same far-right status/Undo columns; no Check-in or QR evidence | Attendance from log derives Present from a completed Pika log when checked; unchecked leaves the baseline Unmarked, and teacher overrides remain reversible in either mode |
