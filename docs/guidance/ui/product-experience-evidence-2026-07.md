...
| Hot archive discovery/actions | Committed pair plus focused CI-backed browser matrix | Safety Wave captures Delete removal. Phase 5 now covers rollout-unavailable, status-outage/retry, available, confirmation, stale, verified, and student-absence states at desktop/mobile in both themes. Compaction and cold-restore progress remain. |
...
| Student grades and profile | Durable no-build decision recorded: returned assignment/test results remain the grade surface; profile collection remains in the join flow | Aggregate grades require returned-only disclosure, weighting, and incomplete-work semantics. Profile editing requires a source-of-truth and synchronization contract across global profiles and classroom roster names. |
| Blueprints | Desktop committed; direct create/rollover and publication lifecycle drills are CI-backed | Phase 5 browser coverage proves the preselected current-Blueprint flow, internal immutable Version lineage, student-facing draft/unpublished defaults, live-data exclusion, and planned-site publish/unpublish behavior. No pre-create preview or Version-selection workflow is planned; Phase 6 owns remaining legacy shell/theme parity. |
| Public `/planned/[slug]` | CI-backed four-project Playwright capture | The seeded matrix generates desktop/mobile light/dark published and generic not-found screenshots and asserts keyboard, overflow, safe-link, and private-content exposure boundaries. |
