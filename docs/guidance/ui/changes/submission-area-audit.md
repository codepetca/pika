...
- Migration `142_allow_acknowledged_missing_assignment_attachments.sql` narrows the database validation function to present-but-invalid attachments so the API can own the acknowledgement boundary. Preparing this file does not authorize applying it to any database.
