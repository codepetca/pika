...
- The submit API requires `allow_missing_attachments` plus the exact confirmed requirement IDs when any configured attachment is absent. A newly missing requirement is never covered by an earlier confirmation. Ownership, revision and immutable-submission checks remain unchanged.
