...
- The submit API requires an explicit `allow_missing_attachments` acknowledgement when any configured attachment is absent. Ownership, revision and immutable-submission checks remain unchanged.
- Migration `144_allow_acknowledged_missing_assignment_attachments.sql` keeps the acknowledgement boundary inside the locked submission transaction while still blocking present invalid attachments. Preparing this file does not authorize applying it to any database.
