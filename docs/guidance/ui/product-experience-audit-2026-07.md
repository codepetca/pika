...
| Roster | `TeacherRosterTab` | roster add/upload/edit/bulk-delete routes and atomic removal RPC; roster/enrollment/profile/user tables | Component, API, parser, removal, and RPC coverage includes cold and retained recovery, stale-request isolation, direct table keyboard behavior, counselor edits, and removal confirmation. | Desktop table selection, confirmation, counselor editing, error recovery, and focus behavior are covered. Mobile still hides email and counselor fields without row detail. |
...
5. Roster: completed desktop keyboard behavior, bulk-action recovery, and counselor-field access; mobile row detail remains deferred.
