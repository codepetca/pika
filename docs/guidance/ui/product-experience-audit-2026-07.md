...
- The `blueprint-rollover` local browser drill now captures the seeded classroom, creates a next-term classroom, verifies reusable titles, artifact lineage, nested assessment content, instructions, lesson content, syllabus/resources, grading configuration, draft/release state, and exclusions for enrollments, roster rows, logs, submissions, and test attempts. It captures the Blueprint review, release-review handoff, and assignment review screens, refuses non-loopback targets, and removes its generated records.
