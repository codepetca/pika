...
| Blueprints | `/teacher/blueprints` | blueprint CRUD/import/export/instantiate/merge/AI routes; operation/package modules; blueprint tables and atomic RPCs | Server, API, package, migration, focused component suites, and a loopback-only browser drill cover stable retry identity, cross-section draft preservation, guarded transitions, saved-version actions, and classroom rollover boundaries. | Dirty sections are explicit and protected across saves, Blueprint changes, route actions, authority changes, imports, proposal application, exports, and classroom creation. The seeded browser drill proves reusable structure/lineage, draft release state, live-data exclusion, and the teacher review handoff. Legacy tabs/shell semantics and planned-site verification remain. |
...
- Immutable package fixtures and a direct/TAR compatibility matrix now cover every supported version (`2`-`5`), including retired Quiz/config normalization, strict v4/v5 boundaries, v5 identity/grading lineage, portable document boundaries, and unique parent/child Artifact IDs. Legacy imports derive Artifact IDs from the operation UUID so same-key retries rebuild the same atomic plan; valid v4 TAR archives retain their declared six-file shape instead of being rejected after decode.
- Remaining Blueprint work includes planned-site verification and broader preview/version workflow productization.
