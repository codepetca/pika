...
- The first archive productization slice is complete. Hot archived Classroom rows now distinguish database-only state, rollout-unavailable recovery copies, interrupted/retryable export, and verified private recovery copies. Eligible teachers can explicitly create or resume the existing idempotent export after a confirmation that no hot data is removed; verified rows show compressed size and retention policy without exposing paths, checksums, identities, or student data. This does not compact the Classroom or free database space. Remaining work starts with hot-to-cold eligibility/progress, then cold-restore progress and quota/retention policy.
