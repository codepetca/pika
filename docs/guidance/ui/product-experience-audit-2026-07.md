...
- The `blueprint-rollover` local browser drill now captures the seeded classroom, creates a next-term classroom, and uses temporary non-empty fixtures to verify assignments, tests, lessons, materials, surveys, requirements, and questions. It checks parent/child artifact identity and immutable Blueprint Version lineage, reusable content and settings, draft/release state, and exclusions for enrollments, roster rows, logs, submissions, attempts, announcements, and announcement reads. It captures the Blueprint review, release-review handoff, and assignment review screens, refuses managed uploads and non-loopback targets, and proves cleanup restores source metadata plus local operation and storage inventories.
