...
| Course Guide/resources | shared authenticated Course Guide panel and public `/actual/[slug]` page | public-safe Course Guide projection over course-site and published classroom sources | Domain, API, component, and browser coverage includes orientation sections, title-only assessment lists, loading, retry, readiness, unpublished, empty, and not-found states. | Teacher, student, and public views share one responsive document presentation without an iframe. The authenticated view retains an external-open escape and explicit loading/error recovery. |
| Settings | classroom settings surface | classroom/calendar/join/blueprint routes and modules | Settings and related route/domain behavior have focused component/API coverage. | General, Access, Class Days, and Reuse are stable URL-backed sections using the shared keyboard-operable segmented control. Course Guide authoring, section visibility, and optional public sharing now stay inside the Course Guide pane; legacy Course Guide settings URLs fall back to General. |
...
| Blueprints | `/teacher/blueprints` | blueprint CRUD/import/export/instantiate/merge/AI routes; operation/package modules; blueprint tables and atomic RPCs | Server, API, package, migration, focused component suites, and browser drills cover stable retry identity, cross-section draft preservation, guarded transitions, saved-version actions, package compatibility, direct current-Blueprint classroom rollover, student-facing draft/unpublished defaults, and planned-site publication. | Dirty sections are explicit and protected across saves, Blueprint changes, route actions, authority changes, imports, proposal application, exports, and classroom creation. `Use for Classroom` uses the current saved Blueprint without a pre-create preview or Version picker; instantiation snapshots the immutable Version internally for lineage. Seeded browser drills prove reusable structure/lineage, student API non-visibility, live-data exclusion, the teacher review handoff, and planned-site publish/unpublish behavior. Legacy tab/shell parity belongs to Phase 6, not a broader preview/version workflow. |
| Public planned-course site | `/planned/[slug]`, opened from Blueprints | public planned-course loader and publishing contract; blueprint publication metadata and course content | Publishing/domain tests plus a seeded four-project Playwright matrix cover source selection, availability, responsive light/dark rendering, generic not-found behavior, keyboard traversal, safe resource links, and private-content exclusion. | The public page now uses a scan-friendly section layout with consistent Test terminology. Published and unpublished states pass desktop/mobile overflow, theme, focus, and content-exposure checks. |
...
| Course Guide and announcements | shared Course Guide panel and announcement section | public-safe Course Guide projection plus announcement/read tables | Course Guide and announcement coverage includes loading/retry/unpublished states and explicit announcement recovery/read behavior. | The guide is a native responsive document in the classroom and on its public URL; announcements retain explicit failures and Toronto timestamps. |
...
Course Guide/resources progress:
...
- The guide is derived live from the classroom projection and stays a high-level orientation document: authored curriculum overview and resources plus compact title-only Assignment and Test lists. Lesson sequence and Announcements remain in their own classroom surfaces and are not part of the guide. The legacy outline field remains stored only for compatibility.
...
10. Course Guide/resources: completed shared native presentation, live classroom projection, orientation-only sections, title-only Assignment/Test lists, readiness, loading, Retry, unpublished, empty, and public not-found behavior.
...
12. Settings and student grades/profile: completed. Classroom Settings is organized into stable General, Access, Class Days, and Reuse sections; Course Guide authoring and sharing remain in the guide pane. A general Gradebook breakdown (for example Attendance, Term Work, and Final) is tracked as future work outside the Course Guide; aggregate student grades and standalone profile editing still require explicit disclosure and data-authority contracts.
...
1. Productize end-of-course rollover: capture a reusable Blueprint and create the next classroom from its current saved state without students, submissions, grades, attendance, or runtime publication state. Instantiation snapshots the immutable Blueprint Version internally for lineage; teachers do not need a pre-create preview or Version picker.
...
- From `/teacher/blueprints`, `Use for Classroom` already owns the source choice, so the teacher supplies the classroom name and required calendar directly. Dashboard/classroom entry paths without a preselected Blueprint retain source selection. The flow intentionally has no pre-create preview or immutable Version picker; `createClassroomFromBlueprint` snapshots the current saved Blueprint and records Version lineage atomically.
...
- The `blueprint-rollover` local browser drill now captures the seeded classroom, creates a next-term classroom through the direct preselected flow, and uses temporary non-empty fixtures to verify assignments, Tests, reusable Test documents/settings, lessons, materials, surveys, requirements, and questions. It checks parent/child artifact and source identity plus immutable Blueprint Version ownership; assignments, Tests, materials, surveys, and the actual classroom site all remain draft/unreleased/unopened/unpublished. An authenticated student browser context proves the new classroom remains unavailable through assignment, Test, material, and survey APIs, while database checks exclude enrollments, roster rows, logs, submissions, Test attempts/responses, announcements, and announcement reads. The drill captures the Blueprint review, release-review handoff, and assignment review screens, refuses managed uploads and non-loopback targets, and proves cleanup restores source metadata/document state while deleting only request-key-owned operation rows and preserving local operation/storage inventories.
- Course Package compatibility evidence, direct Blueprint-to-Classroom creation, and public planned-site verification are complete. A broader pre-create preview/version workflow is intentionally not planned. Remaining Phase 5 work is the archive lifecycle productization in items 5–9: teacher-visible export/verification/compaction/restore progress and recovery, plus retention/quota/purge policy states. New active Classrooms continue through the existing archive/purge lifecycle; this flow does not promise immediate permanent deletion.
- The first archive productization slice is complete pending migration 126 rollout. Hot archived Classroom rows now distinguish database-only state, rollout-unavailable or temporarily unreadable recovery status, interrupted/retryable export, stale copies from an earlier Classroom revision, and current verified private recovery copies. Eligible teachers can explicitly create or resume the existing idempotent export after a confirmation that no hot data is removed; same-lifecycle tabs use one deterministic operation ID, retries preserve the original retention contract, and migration 126 atomically rejects stale-tab revisions before operation creation. Verified rows show compressed size and retention policy without exposing paths, checksums, identities, or student data. This does not compact the Classroom or free database space. Remaining work starts with hot-to-cold eligibility/progress, then cold-restore progress and quota/retention policy.
