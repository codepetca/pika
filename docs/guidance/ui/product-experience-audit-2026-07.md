...
| Blueprints | `/teacher/blueprints` | blueprint CRUD/import/export/instantiate/merge/AI routes; operation/package modules; blueprint tables and atomic RPCs | Server, API, package, migration, focused component suites, and browser drills cover stable retry identity, cross-section draft preservation, guarded transitions, saved-version actions, package compatibility, classroom rollover boundaries, and planned-site publication. | Dirty sections are explicit and protected across saves, Blueprint changes, route actions, authority changes, imports, proposal application, exports, and classroom creation. Seeded browser drills prove reusable structure/lineage, draft release state, live-data exclusion, the teacher review handoff, and planned-site publish/unpublish behavior. Legacy tabs/shell semantics and broader preview/version workflow productization remain. |
...
- The `blueprint-rollover` local browser drill now captures the seeded classroom, creates a next-term classroom, and uses temporary non-empty fixtures to verify assignments, tests, reusable test documents/settings, lessons, materials, surveys, requirements, and questions. It checks parent/child artifact and source identity plus immutable Blueprint Version ownership, draft/release state, and exclusions for enrollments, roster rows, logs, submissions, test attempts/responses, announcements, and announcement reads. It captures the Blueprint review, release-review handoff, and assignment review screens, refuses managed uploads and non-loopback targets, and proves cleanup restores source metadata/document state while deleting only request-key-owned operation rows and preserving local operation/storage inventories.
- Course Package compatibility evidence and public planned-site verification are complete. Remaining Blueprint work is broader preview/version workflow productization.
