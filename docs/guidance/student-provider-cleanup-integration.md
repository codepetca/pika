...
| Local attendance facts/projections/overrides/events and shared override requests | Explicitly inventoried or classified as blockers pending a local attendance cleanup contract |
...
Verification authored: orchestration/storage mocks and the rollback-only
`scripts/check-removed-student-academic-database.sql` harness. The harness requires
approved local173 and changes only synthetic transaction-local fixtures; it must
