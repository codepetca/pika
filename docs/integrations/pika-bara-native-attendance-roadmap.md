...
1. An occurrence QR opens `/attendance/check-in/<opaque-pika-token>` on Pika.
   ...
   Pika requires attendance access and resolves an eligible open occurrence
   server-side after enrollment and participant checks. The poster remains
   displayable outside attendance hours, while scan-time eligibility remains
   fail-closed. See [stable classroom QR](../guidance/classroom-attendance-qr.md).
