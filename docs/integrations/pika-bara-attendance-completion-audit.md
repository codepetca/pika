...
| Archive/purge containment | Complete as a fail-closed interim boundary. Soft archive/restore retains attendance state. Every attendance row family, compaction start, purge start, and final classroom delete is guarded until a versioned Bara decommission/reseed protocol exists. Inbox/projections carry local classroom lineage and record projections carry student lineage. | Destructive archive/purge remains intentionally unavailable for attendance-enabled classrooms. |
