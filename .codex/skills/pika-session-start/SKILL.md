...
1. Run the session start script from the worktree root:
   ...
   ```
2. Review the output — confirm worktree path contains `.codex/worktrees/` (or `.claude/worktrees/`), check branch and task.
3. If an issue number is provided, load it with `gh issue view <number>`.
4. Propose implementation plan before writing any code.

## Guardrails

- STOP if `git rev-parse --show-toplevel` does not contain `.codex/worktrees/` or `.claude/worktrees/` (wrong directory).
