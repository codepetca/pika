-- Repo-linked assignment grading: feedback history, repo targets, analysis runs/results
...
create table if not exists public.assignment_repo_targets (
  ...
  override_github_username text,
  repo_owner text,
  repo_name text,
  selection_mode text not null default 'auto'
    check (selection_mode in ('auto', 'teacher_override')),
