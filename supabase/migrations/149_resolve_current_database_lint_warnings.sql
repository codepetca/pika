...
create function private.replace_function_definition_fragment_v149(
...
begin
  if p_expected is null or p_expected = '' then
    raise exception 'Migration 149 replacement fragment must not be empty'
      using errcode = '22023';
  end if;
  ...
  if v_occurrences <> 1 then
    raise exception 'Migration 149 expected exactly one fragment in %, found %',
      p_function::text,
      v_occurrences
      using errcode = '55000';
  end if;
...
revoke all on function private.replace_function_definition_fragment_v149(
  regprocedure,
  text,
  text
...
select private.replace_function_definition_fragment_v149(
