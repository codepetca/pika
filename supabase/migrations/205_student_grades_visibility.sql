...
alter function public.normalize_classroom_archive_restore_row(uuid, text, jsonb)
  rename to normalize_classroom_archive_restore_row_pre_v205;

revoke all on function public.normalize_classroom_archive_restore_row_pre_v205(uuid, text, jsonb)
...
as $$
begin
  p_row := public.normalize_classroom_archive_restore_row_pre_v205(
    p_operation_id,
    p_table_name,
    p_row
  );
