-- Serialize the dormant contextual save boundary with the established
-- assignment submission and editor-save transactions before taking classroom
-- membership fences. This prevents a save from holding classroom/student locks
-- while an existing submit, unsubmit, restore or autosave owns the document.
...
comment on function public.save_assignment_doc_for_member_v1(
  ...
  'Dormant service-only assignment save boundary: serializes document mutations before holding current membership and live assignment locks.';
