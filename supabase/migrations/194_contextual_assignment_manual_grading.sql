-- Add a dormant, service-only contextual boundary for manual Assignment grades.
-- Exact user/Assignment API admission remains independently disabled by default.

begin;

create function public.save_assignment_grades_for_owner_v1(
  p_actor_id uuid,
...
begin
  ...
  if v_archived_at is not null or v_blueprint_archived_at is not null then
    raise exception using errcode = '55000', message = 'Assignment is archived';
  end if;
...
comment on function public.save_assignment_grades_for_owner_v1(
  ...
  'Fences and rechecks the current active Assignment owner before delegating one manual grade save.';
