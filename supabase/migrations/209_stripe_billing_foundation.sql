...
create table public.stripe_billing_event_inbox (
  ...
  status text not null default 'received' check (status in ('received', 'completed', 'exception')),
  exception_code text,
...
create table public.stripe_billing_subscription_audit (
  ...
  event_inbox_id uuid,
  outcome text not null check (outcome in ('paid', 'noop', 'exception', 'failed')),
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_event public.stripe_billing_event_inbox%rowtype; v_binding_id uuid; v_status text;
begin
  perform private.assert_stripe_billing_sandbox_enabled_v1();
  if p_request is null or jsonb_typeof(p_request) <> 'object'
    or coalesce(p_request->>'payload_hash','') !~ '^[a-f0-9]{64}$' or coalesce(p_request->>'event_id','') !~ '^evt_[A-Za-z0-9]{1,255}$' or coalesce(p_request->>'stripe_account','') !~ '^acct_[A-Za-z0-9]{1,255}$' or coalesce(p_request->>'event_type','') = ''
    or p_request->>'received_at' is null or jsonb_typeof(p_request->'payload') <> 'object' then
    raise exception using errcode='22023', message='stripe_billing_event_request_invalid';
  end if;
  select id into v_binding_id from public.stripe_billing_subscription_bindings
  where stripe_account=p_request->>'stripe_account' and provider_mode='test' and stripe_subscription_id=p_request->'payload'->>'subscription_id';
  v_status := case when v_binding_id is null then 'exception' else 'received' end;
  insert into public.stripe_billing_event_inbox (stripe_account,provider_mode,stripe_event_id,payload_hash,event_type,event_created_at,subscription_id,stripe_customer_id,stripe_subscription_id,stripe_invoice_id,stripe_payment_intent_id,status,exception_code,next_attempt_at)
  values (p_request->>'stripe_account','test',p_request->>'event_id',p_request->>'payload_hash',p_request->>'event_type',(p_request->>'received_at')::timestamptz,v_binding_id,p_request->'payload'->>'customer_id',p_request->'payload'->>'subscription_id',p_request->'payload'->>'object_id',null,v_status,case when v_binding_id is null then 'unbound_event' end,case when v_binding_id is null then clock_timestamp() + interval '5 minutes' else clock_timestamp() end)
  on conflict (stripe_account,provider_mode,stripe_event_id) do nothing returning * into v_event;
  if v_event.id is null then
    select * into v_event from public.stripe_billing_event_inbox where stripe_account=p_request->>'stripe_account' and provider_mode='test' and stripe_event_id=p_request->>'event_id';
    if v_event.payload_hash <> p_request->>'payload_hash' then raise exception using errcode='23505', message='stripe_billing_event_conflict'; end if;
...
returns jsonb language sql stable security definer set search_path = '' as $$
  with params as (select greatest(1,least(50,coalesce((p_request->>'limit')::integer,0))) as limit_value),
  candidates as (
    select 'event'::text as kind, event.id as event_inbox_id, event.subscription_id, event.next_attempt_at
    from public.stripe_billing_event_inbox event where event.status in ('received','exception') and event.subscription_id is not null and event.next_attempt_at <= clock_timestamp()
    union all
    select 'reconcile'::text, null, binding.id, binding.next_reconcile_at
    from public.stripe_billing_subscription_bindings binding where binding.next_reconcile_at <= clock_timestamp()
  ), picked as (select distinct on (subscription_id) * from candidates order by subscription_id, kind, next_attempt_at limit (select limit_value from params))
  select jsonb_build_object('items',coalesce(jsonb_agg(jsonb_build_object('kind',picked.kind,'event_inbox_id',picked.event_inbox_id,'subscription_id',binding.id,'binding',jsonb_build_object('subscription_id',binding.id,'subject_user_id',binding.subject_user_id,'stripe_account',binding.stripe_account,'stripe_customer_id',binding.stripe_customer_id,'stripe_subscription_id',binding.stripe_subscription_id,'offering_id',offering.id,'offering_version_id',version.id,'stripe_price_id',version.stripe_price_id,'plan_key',offering.plan_key,'currency',version.currency,'interval',version.interval,'provider_mode',binding.provider_mode),'next_attempt_at',picked.next_attempt_at)), '[]'::jsonb))
  from picked join public.stripe_billing_subscription_bindings binding on binding.id=picked.subscription_id join public.stripe_billing_offering_versions version on version.id=binding.offering_version_id join public.stripe_billing_offerings offering on offering.id=version.offering_id;
$$;

create function public.billing_claim_subscription_v1(p_request jsonb)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_binding public.stripe_billing_subscription_bindings%rowtype; v_plan public.account_plans%rowtype; v_version public.stripe_billing_offering_versions%rowtype; v_offering public.stripe_billing_offerings%rowtype; v_seconds integer; v_token uuid := gen_random_uuid();
begin
  perform private.assert_stripe_billing_sandbox_enabled_v1();
  v_seconds := (p_request->>'lease_seconds')::integer;
  if p_request is null or jsonb_typeof(p_request) <> 'object' or (p_request->>'subscription_id') is null or v_seconds is null or v_seconds not between 30 and 300 then raise exception using errcode='22023',message='stripe_billing_claim_request_invalid'; end if;
  select * into v_binding from public.stripe_billing_subscription_bindings where id=(p_request->>'subscription_id')::uuid for update;
  if not found then return jsonb_build_object('status','not_found'); end if;
  if v_binding.lease_expires_at is not null and v_binding.lease_expires_at > clock_timestamp() then return jsonb_build_object('status','busy'); end if;
  ...
  update public.stripe_billing_subscription_bindings set lease_token=v_token,lease_expires_at=clock_timestamp()+make_interval(secs=>v_seconds),fencing_token=fencing_token+1,revision=revision+1,updated_at=clock_timestamp() where id=v_binding.id returning * into v_binding;
  return jsonb_build_object('status','claimed','subscription_id',v_binding.id,'lease_token',v_token,'fencing_token',v_binding.fencing_token,'lease_expires_at',v_binding.lease_expires_at,'subscription_revision',v_binding.revision,'expected_account_plan_revision',v_plan.revision,'binding',jsonb_build_object('subscription_id',v_binding.id,'subject_user_id',v_binding.subject_user_id,'stripe_account',v_binding.stripe_account,'stripe_customer_id',v_binding.stripe_customer_id,'stripe_subscription_id',v_binding.stripe_subscription_id,'offering_id',v_offering.id,'offering_version_id',v_version.id,'stripe_price_id',v_version.stripe_price_id,'plan_key',v_offering.plan_key,'currency',v_version.currency,'interval',v_version.interval,'provider_mode',v_binding.provider_mode));
end; $$;

create function public.billing_finish_subscription_v1(p_request jsonb)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_binding public.stripe_billing_subscription_bindings%rowtype; v_plan public.account_plans%rowtype; v_version public.stripe_billing_offering_versions%rowtype; v_entitlement_revision bigint; v_entitlement jsonb; v_event public.stripe_billing_event_inbox%rowtype; v_invoice public.stripe_billing_invoice_effects%rowtype; v_operation uuid := gen_random_uuid(); v_outcome text; v_reason text; v_previous_plan_key text;
begin
  perform private.assert_stripe_billing_sandbox_enabled_v1();
  v_outcome := p_request->>'outcome'; v_reason := p_request->>'reason_code';
  if p_request is null
    or jsonb_typeof(p_request) <> 'object'
    or (p_request->>'subscription_id') is null
    or (p_request->>'lease_token') is null
    ...
    or v_outcome not in ('paid','noop','exception')
  ...
  select * into v_plan from public.account_plans where subject_user_id=v_binding.subject_user_id for update;
  if not found then return jsonb_build_object('status','rejected','retry_scheduled',false); end if;
  if v_binding.lease_token is distinct from (p_request->>'lease_token')::uuid or v_binding.lease_expires_at <= clock_timestamp() or v_binding.fencing_token <> (p_request->>'fencing_token')::bigint or v_binding.revision <> (p_request->>'expected_subscription_revision')::bigint then return jsonb_build_object('status','lost_claim','retry_scheduled',true); end if;
  if v_plan.revision <> (p_request->>'expected_account_plan_revision')::bigint then return jsonb_build_object('status','plan_conflict','retry_scheduled',true); end if;
  if p_request->>'event_inbox_id' is not null then select * into v_event from public.stripe_billing_event_inbox where id=(p_request->>'event_inbox_id')::uuid and subscription_id=v_binding.id for update; if not found then return jsonb_build_object('status','rejected','retry_scheduled',false); end if; end if;
  if v_outcome='paid' then
    if p_request->>'invoice_id' is null or p_request->>'period_start' is null or p_request->>'period_end' is null or (p_request->>'period_end')::timestamptz <= (p_request->>'period_start')::timestamptz or (v_binding.last_period_end is not null and (p_request->>'period_end')::timestamptz < v_binding.last_period_end) then return jsonb_build_object('status','rejected','retry_scheduled',false); end if;
    perform pg_advisory_xact_lock(hashtextextended('stripe-invoice:' || v_binding.stripe_account || ':' || (p_request->>'invoice_id'), 20920260926));
    select * into v_invoice from public.stripe_billing_invoice_effects where stripe_account=v_binding.stripe_account and stripe_invoice_id=p_request->>'invoice_id';
    if found then
      if v_invoice.subscription_id <> v_binding.id or v_invoice.offering_version_id <> v_binding.offering_version_id or v_invoice.period_start <> (p_request->>'period_start')::timestamptz or v_invoice.period_end <> (p_request->>'period_end')::timestamptz then return jsonb_build_object('status','rejected','retry_scheduled',false); end if;
      if v_plan.management_source <> 'billing' or v_plan.billing_offering_version_id <> v_invoice.offering_version_id or v_plan.revision <> v_invoice.account_plan_revision or not exists (select 1 from public.effective_feature_entitlements entitlement join public.stripe_billing_offering_versions version on version.id=v_invoice.offering_version_id where entitlement.subject_user_id=v_binding.subject_user_id and entitlement.feature_key='classrooms.create' and entitlement.source='plan' and entitlement.enabled=(version.classroom_limit>0) and entitlement.quota_limit=version.classroom_limit) then return jsonb_build_object('status','plan_conflict','retry_scheduled',true); end if;
      ...
      update public.stripe_billing_subscription_bindings set lease_token=null,lease_expires_at=null,next_reconcile_at=clock_timestamp()+interval '1 day',revision=revision+1,updated_at=clock_timestamp() where id=v_binding.id;
    ...
    insert into public.account_plan_audit (operation_id,subject_user_id,previous_plan_key,new_plan_key,new_classroom_limit,plan_revision,entitlement_revision,actor_ref,reason_code,request_fingerprint) values (v_operation,v_binding.subject_user_id,v_previous_plan_key,v_plan.plan_key,v_version.classroom_limit,v_plan.revision,(v_entitlement->>'revision')::bigint,'stripe:billing','stripe_paid',md5(jsonb_build_object('subscription_id',v_binding.id,'invoice_id',p_request->>'invoice_id')::text));
    insert into public.stripe_billing_invoice_effects (subscription_id,stripe_account,stripe_invoice_id,offering_version_id,account_plan_revision,period_start,period_end,event_inbox_id) values (v_binding.id,v_binding.stripe_account,p_request->>'invoice_id',v_version.id,v_plan.revision,(p_request->>'period_start')::timestamptz,(p_request->>'period_end')::timestamptz,v_event.id);
  end if;
  ...
  set lease_token = null,
      ...
      last_exception_code = case when v_outcome='exception' then coalesce(v_reason,'unknown') else null end,
      next_reconcile_at = clock_timestamp()+case when v_outcome='exception' then interval '5 minutes' else interval '1 day' end,
      revision = revision+1,
      updated_at = clock_timestamp()
  where id=v_binding.id
  returning * into v_binding;
  if v_event.id is not null then update public.stripe_billing_event_inbox set status=case when v_outcome='exception' then 'exception' else 'completed' end,exception_code=case when v_outcome='exception' then coalesce(v_reason,'unknown') end,next_attempt_at=case when v_outcome='exception' then clock_timestamp()+interval '5 minutes' else null end,completed_at=case when v_outcome='exception' then null else clock_timestamp() end where id=v_event.id; end if;
  insert into public.stripe_billing_subscription_audit (subscription_id,event_inbox_id,outcome,subscription_revision,account_plan_revision,offering_version_id,reason_code) values (v_binding.id,v_event.id,v_outcome,v_binding.revision,case when v_outcome='paid' then v_plan.revision else null end,case when v_outcome='paid' then v_binding.offering_version_id else null end,v_reason);
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_binding public.stripe_billing_subscription_bindings%rowtype;
begin
  perform private.assert_stripe_billing_sandbox_enabled_v1();
  if p_request is null or jsonb_typeof(p_request)<>'object' or (p_request->>'subscription_id') is null or (p_request->>'lease_token') is null or coalesce(p_request->>'fencing_token','') !~ '^[1-9][0-9]*$' or coalesce(p_request->>'expected_subscription_revision','') !~ '^[1-9][0-9]*$' or coalesce(p_request->>'error_code','') !~ '^[a-z][a-z0-9._-]{0,99}$' or p_request->>'retry_at' is null then raise exception using errcode='22023',message='stripe_billing_fail_request_invalid'; end if;
  select * into v_binding from public.stripe_billing_subscription_bindings where id=(p_request->>'subscription_id')::uuid for update;
  if not found or v_binding.lease_token is distinct from (p_request->>'lease_token')::uuid or v_binding.lease_expires_at <= clock_timestamp() or v_binding.fencing_token<>(p_request->>'fencing_token')::bigint or v_binding.revision<>(p_request->>'expected_subscription_revision')::bigint then return jsonb_build_object('status','lost_claim'); end if;
  update public.stripe_billing_subscription_bindings set lease_token=null,lease_expires_at=null,last_exception_code=p_request->>'error_code',next_reconcile_at=(p_request->>'retry_at')::timestamptz,revision=revision+1,updated_at=clock_timestamp() where id=v_binding.id returning * into v_binding;
  insert into public.stripe_billing_subscription_audit (subscription_id,outcome,subscription_revision,reason_code) values (v_binding.id,'failed',v_binding.revision,p_request->>'error_code');
