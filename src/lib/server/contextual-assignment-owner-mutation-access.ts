...
export type ContextualAssignmentOwnerMutationAccess =
...
function configuredAssignmentPairs(): z.infer<typeof assignmentPairsSchema> | null {
  const raw = process.env.PIKA_CLASSROOM_ASSIGNMENT_OWNER_MUTATION_ACCESS_PAIRS
...
/** Dormant exact-pair admission for owner mutations of an existing assignment. */
export async function authorizeContextualAssignmentOwnerMutationRequest(
  assignmentId: string | (() => string | Promise<string>),
): Promise<ContextualAssignmentOwnerMutationAccess> {
  if (process.env.PIKA_CLASSROOM_ASSIGNMENT_OWNER_MUTATION_ACCESS_ENABLED !== 'true') {
  ...
  if (pairs === null || !identity.success) {
    throw new ApiError(503, 'Classroom assignment owner mutation configuration is unavailable')
  }
