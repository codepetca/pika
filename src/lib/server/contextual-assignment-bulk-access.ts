...
export type ContextualAssignmentBulkAccess =
...
function configuredClassroomPairs(): z.infer<typeof classroomPairsSchema> | null {
  const raw = process.env.PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_PAIRS
...
/** Dormant exact-pair admission for the markdown Assignment bulk editor. */
export async function authorizeContextualAssignmentBulkRequest(
  classroomId: string | (() => string | Promise<string>),
): Promise<ContextualAssignmentBulkAccess> {
  if (process.env.PIKA_CLASSROOM_ASSIGNMENT_BULK_ACCESS_ENABLED !== 'true') {
  ...
  if (pairs === null || !identity.success) {
    throw new ApiError(503, 'Classroom assignment bulk configuration is unavailable')
  }
