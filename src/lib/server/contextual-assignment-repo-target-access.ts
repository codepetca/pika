...
function configuredAssignmentPairs(): z.infer<typeof assignmentPairsSchema> | null {
  const raw = process.env.PIKA_CLASSROOM_ASSIGNMENT_REPO_TARGET_ACCESS_PAIRS
...
export async function authorizeContextualAssignmentRepoTargetRequest(
  ...
  if (pairs === null || !identity.success) {
    throw new ApiError(503, 'Classroom assignment repo target configuration is unavailable')
  }
