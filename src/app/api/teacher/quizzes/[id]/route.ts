...
export const GET = withErrorHandler('GetTeacherQuiz', async (request, context) => {
...
export const PATCH = withErrorHandler('UpdateTeacherQuiz', async (request, context) => {
  const user = await requireRole('teacher')
  const { id } = await context.params
...
export const DELETE = withErrorHandler('DeleteTeacherQuiz', async (request, context) => {
  const user = await requireRole('teacher')
  const { id } = await context.params
