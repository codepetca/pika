...
type PurgeResponse = {
  impact?: CourseBlueprintPurgeImpact
  operation?: CourseBlueprintPurgeStatus | null
  advanced?: boolean
  error?: string
}
...
export function CourseBlueprintPurgeDialog({
  courseBlueprintId,
  ...
  const [impact, setImpact] = useState<CourseBlueprintPurgeImpact | null>(null)
  const [operation, setOperation] = useState<CourseBlueprintPurgeStatus | null>(null)
  ...
  const isConfirmed = confirmation === 'DELETE'
    || confirmation === courseBlueprintTitle
  const totalFiles = useMemo(
    () => operation
      ? Object.values(operation.storage_object_counts)
  ...
  useEffect(() => {
    ...
    fetchJSON<PurgeResponse>(
      `/api/teacher/course-blueprints/${courseBlueprintId}/purge`,
      {
        init: { cache: 'no-store' },
      ...
      .then((body) => {
        ...
        pendingOperationIdRef.current = body.operation?.operation_id || null
      })
      .catch((reason: unknown) => {
        if (!mountedRef.current) return
        setError(reason instanceof Error
          ? reason.message
          : 'Could not prepare permanent deletion')
      })
      .finally(() => { if (mountedRef.current) setIsLoading(false) })
  }, [courseBlueprintId, isOpen])

  async function runUntilSettled(initial: CourseBlueprintPurgeStatus) {
    let current = initial
    ...
    for (let tick = 0; tick < 10_000 && mountedRef.current; tick += 1) {
      ...
      if (current.status === 'failed' && current.retryable === false) {
        throw new Error(
          'Permanent deletion stopped safely before the Blueprint database records were removed.',
        )
      }
      const response = await fetch(
        `/api/teacher/course-blueprints/${courseBlueprintId}/purge/${current.operation_id}/tick`,
        { method: 'POST' },
      )
      ...
      if ((current.storage_object_counts.failed || 0) > 0) {
        throw new Error(
          'A file deletion failed safely. Progress was saved; select Continue deletion to retry.',
        )
      }
    }
        ...
        const response = await fetch(
          `/api/teacher/course-blueprints/${courseBlueprintId}/purge`,
          {
            ...
            body: JSON.stringify({
              ...
              expected_inventory_sha256: impact?.inventory_sha256,
            }),
          },
        )
        const body = await response.json() as PurgeResponse
  ...
  return (
    <ContentDialog
      isOpen={isOpen}
      onClose={close}
      title="Delete Course Blueprint permanently?"
      subtitle={courseBlueprintTitle}
      maxWidth="max-w-xl"
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-text-default">
                  This permanently removes the Course Blueprint content, saved Versions,
                  assignments, tests, lesson plans, materials, surveys, and uploaded
                  teacher test files.
                </p>
                <p className="mt-2 text-sm text-text-muted">
                  Linked Classrooms, their student work and file copies, and all user
                  accounts are kept. Their Blueprint connection is removed.
                </p>
                {impact?.planned_site_published ? (
                  <p className="mt-2 text-sm font-medium text-text-default">
                    Its published planned course site will also be removed.
                  </p>
                ) : null}
              </div>
            </div>
          ...
          {impact ? (
            <div>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                ...
                <div className="rounded-control border border-border bg-surface-2 p-2 sm:p-3">
                  <School className="h-4 w-4 text-text-muted" aria-hidden="true" />
                  <p className="mt-2 text-lg font-semibold text-text-default">
                    {impact.linked_classroom_count}
                  </p>
                  <p className="text-xs text-text-muted">classrooms kept</p>
                </div>
                <div className="rounded-control border border-border bg-surface-2 p-2 sm:p-3">
          ...
          {operation ? (
            ...
            <FormField
              label={`Type “${courseBlueprintTitle}” or DELETE to confirm`}
              hint="The Course Blueprint name is case-sensitive."
            >
              <Input
                value={confirmation}
                autoComplete="off"
          ...
          {impact?.conflicting_operation ? (
            <p className="rounded-control border border-warning bg-warning-bg px-3 py-2 text-sm text-text-default">
              Finish the active Blueprint operation before deleting permanently.
            </p>
          ) : null}
          {impact && !impact.deletion_available ? (
            <p className="rounded-control border border-warning bg-warning-bg px-3 py-2 text-sm text-text-default">
              {impact.unavailable_reason
                || 'Permanent deletion is not available for this Course Blueprint yet.'}
            </p>
          ) : null}
          {error ? (
            <p
          ...
          <div className="flex flex-row justify-end gap-2">
            ...
            <Button
              ...
              disabled={
                isLoading
                || (!operation && impact?.deletion_available === false)
                || (!operation && Boolean(impact?.conflicting_operation))
